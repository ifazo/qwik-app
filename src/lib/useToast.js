...
export const tostifyCustomClose = (el) => {
...
export const showCustomToast = (type, message) => {
  ...
  const markup = `
    <div class="max-w-xs ${color.bg} ${color.border} ${
    color.text
  } text-sm rounded-lg border relative" role="alert">
      <div class="flex p-4 items-start">
        <div class="flex-1 pr-6">
          ${message ?? "Hello, world! This is a toast message."}
        </div>
        <button onclick="tostifyCustomClose(this)" type="button"
          class="absolute top-3 right-3 inline-flex shrink-0 justify-center items-center size-5 rounded-lg ${
            color.close
          } opacity-50 hover:opacity-100 focus:outline-hidden focus:opacity-100"
          aria-label="Close">
